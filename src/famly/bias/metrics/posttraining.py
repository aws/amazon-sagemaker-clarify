...
@registry.posttraining
def FT(df: pd.DataFrame, feature: pd.Series, positive_predicted_label_index: pd.Series) -> float:
    """
    Flip Test (FT)

    :param df: array of data points
    :param feature: boolean facet column indicating sensitive group
    ...
    n_neighbors = 5 if np.array(data_a[0]).size > 10 else 1
